...
用在拼接 html字符串时，因为对str的添加了换行符，使得字符串长度增加而发生变化，但是不影响 html拼接。



splice方法的深入理解：

splice(0, 3) 从第0位开始，截取三个。如果没有第二个参数，从第0位开始，截取到末尾
...
Arguments不是从形参中获取数据，他和形参一样，都是去实参获取数据，只是在ES5中，函数体中，对他俩进行了捆绑，改变一个，另一个也同样改变。而严格模式和ES6的特殊语法中，这种捆绑关系消失，将不再相互影响。



**Array作为函数**：length值和索引值，决定了结果。——Number、String都有用，Array也有作用：
...
箭头函数的目的就是简化执行体，而让让this一目了然。谁的函数，this就是谁，不管你使用call、apply还是bind，都无法改变。因为this已经在箭头函数被创建时，确定。
...
- 箭头函数的this是外层第一个普通函数的this，且无法别改变。
...
RegExp对象：https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/RegExp

接收动态的字符串匹配片段

匹配模式：g全局、i  不区分大小写、m多行匹配
...
可迭代协议：在一个 [`for..of`](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Reference/Statements/for...of) 结构中，哪些值可以被遍历到。并且有默认的迭代行为
...
可以通过提供自己的 `@@iterator` 方法，重新定义迭代行为：**——展开运算符、for of都是通过可迭代对象找到迭代器对象的值**
